...
namespace UserManagement.Tests.Services
{
    ...
    {        private readonly Mock<IUserRepository> _mockUserRepository;
        ...
        private readonly UserService _userService;public UserServiceTests()
        {
            _mockUserRepository = new Mock<IUserRepository>();
            _mockLogger = new Mock<ILogger<UserService>>();
            
            // Create a simple HttpClient for RandomUserApiService (tests won't make actual HTTP calls)
            var httpClient = new HttpClient();
            var randomUserApiService = new RandomUserApiService(httpClient);
        ...
        public async Task GetUsersAsync_WhenCacheHasUsers_ShouldReturnPaginatedResult()
        {
            ...
            _mockUserRepository.Setup(r => r.GetUsersAsync(1, 2)).ReturnsAsync(users);            // Act
            var result = await _userService.GetUsersAsync(1, 2);            // Assert
        ...
        {
            // Arrange
            var searchTerm = "John";            var users = new List<User>
            {
                new User 
                { 
                    Id = Guid.NewGuid(), 
